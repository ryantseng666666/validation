...
import com.fix.cmhk.validation.service.SpeedTestRecognitionService;
import com.fix.cmhk.validation.service.parser.impl.ZhipuSpeedTestParser;
...
public class ZhipuSpeedTestRecognitionService implements SpeedTestRecognitionService {
    
    private final ZhipuAIConfig config;
    private final RestTemplate restTemplate;
    private final ObjectMapper objectMapper;
    private final TokenGenerator tokenGenerator;
    private final ZhipuSpeedTestParser responseParser;
    
    @Override
    public SpeedTestResponse recognizeSpeedTest(String base64Image) {
        try {
            log.info("开始速度测试识别流程...");
            ...
            log.error("速度测试识别过程中发生异常: {}", e.getMessage(), e);
     ...
     * 创建错误响应
     */
    private SpeedTestResponse createErrorResponse(String message) {
