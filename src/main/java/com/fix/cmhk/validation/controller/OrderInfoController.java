...
public class OrderInfoController {
    ...
    public ResponseEntity<String> processMonthlyAI(@RequestParam String monthDate) {
        try {
            return ResponseEntity.ok(orderInfoService.processMonthlyData(monthDate));
        } catch (Exception e) {
            log.error("Process monthly AI data failed for {}: {}", monthDate, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("fail");
        }
    }

    @PostMapping("/check-monthly-duplicates")
    public ResponseEntity<String> checkMonthlyDuplicates(@RequestParam String monthDate) {
        try {
            return ResponseEntity.ok(orderInfoService.checkDuplicates(monthDate));
        } catch (Exception e) {
            log.error("Check monthly duplicates failed for {}: {}", monthDate, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("fail");
        }
    }
} 